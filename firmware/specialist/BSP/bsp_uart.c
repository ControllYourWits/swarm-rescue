...
void BSP_UART3_Init(void){
    ...
    GPIOB->MODER&=~((3U<<20)|(3U<<22));GPIOB->MODER|=(2U<<20)|(2U<<22);
    ...
    DMA1_Stream3->CR=0;DMA1_Stream3->PAR=(uint32_t)&USART3->DR;
...
void BSP_UART3_SendDMA(const uint8_t *buf,uint16_t len){
    while(DMA1_Stream3->CR&DMA_SxCR_EN);USART3->SR;
    DMA1_Stream3->M0AR=(uint32_t)buf;DMA1_Stream3->NDTR=len;
    DMA1_Stream3->CR|=DMA_SxCR_EN;}
void USART3_IRQHandler(void){if(USART3->SR&USART_SR_RXNE)BSP_UART3_RxCB((uint8_t)(USART3->DR&0xFF));}
void DMA1_Stream3_IRQHandler(void){
    if(DMA1->LISR&DMA_LISR_TCIF3){DMA1->LIFCR=DMA_LIFCR_CTCIF3;DMA1_Stream3->CR&=~DMA_SxCR_EN;}}
